...
namespace Maengelruege
{
    public partial class Form1 : Form
    {
        ...
        string sn;
        string[] account = new string[3];  // Increase array size to 3
        bool verification = false;

        private void enterun_Click(object sender, EventArgs e)
        {
            account[0] = Username.Text;
        }

        private void enterp_Click(object sender, EventArgs e)
        {
            account[1] = Password.Text;
        }

        private void entersh_Click(object sender, EventArgs e)
        {
            account[2] = shippingnumber.Text;

            // Validate if username, password, and shipping number are not empty
            if (!string.IsNullOrEmpty(account[0]) && !string.IsNullOrEmpty(account[1]) && !string.IsNullOrEmpty(account[2]))
            {
                if (string.Equals(account[0], username) && string.Equals(account[1], password))
                {
        ...
        string[] Fragen = { "Wie hast du es gekauft?", "Wann has" +
                "t du es gekauft?", "Wie ist der Zustand des Produktes?" };
        ...
        private void Abschicken_Click_1(object sender, EventArgs e)
        {
            ...
            if (accept == true)
            {
                ...
                mailMessage.To.Add(new MailAddress("thebosslife33@gmail.com"));
